...
def AnDA_analog_forecasting(x, AF):
    ...
    while stop_condition != 1:
        ...
        kdt = KDTree(
            AF.catalog.analogs[:, i_var_neighboor], leaf_size=50, metric="euclidean"
        )
        ...
        for i_N in range(0, N):
            ...
            if AF.regression == "locally_constant":
                ...
                cov_xf = (
                    ...
                    * np.dot(
                        np.repeat(weights[i_N, :][np.newaxis], len(i_var), 0) * E_xf,
                        E_xf.T,
                    )
                )
                ...
                mean_x = np.sum(
                    ...
                    * np.repeat(weights[i_N, :][np.newaxis].T, len(i_var_neighboor), 1),
                    0,
                )
