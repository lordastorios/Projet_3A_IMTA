...
def get_traj_with_parcels(date, runtime, delta_time, particle_grid_step, stream_data_fname):
    ...
    pSet = ParticleSet.from_list(fieldset, ScipyParticle, init_longitudes,
                                 init_latitudes, depth=None, time=None)

    # Initialize output file and run simulation
    def DeleteParticle(particle, fieldset, time):
        particle.delete()
    output = pSet.ParticleFile(name="trajectories_temp.nc",outputdt=delta(hours=delta_time))
    ...
    trajectories = np.zeros((nc_traj.dimensions['traj'].size,
                             nc_traj.dimensions['obs' ].size,2))
    trajectories[:,:,0] = np.array(nc_traj['lon'])
    trajectories[:,:,1] = np.array(nc_traj['lat'])
    stream_line_list = []
    for trajectory in trajectories:
        stream_line_list.append(StreamLine(trajectory[np.isfinite(trajectory[:,0])],
                                           delta_time*3600))
...
def get_traj_with_scipy(date, runtime, max_delta_time, particle_grid_step, stream_data_fname, R = EQUATORIAL_EARTH_RADIUS):
    ...
    for sl_id in range(nb_stream_line):
        ...
        dt_list    = []
        previous_t = 0
        integration_instance = sp_integ.RK45(stream, 0, init_pos[sl_id],
            runtime*3600,rtol=1e-4,atol=1e-7,max_step=max_delta_time*3600)
